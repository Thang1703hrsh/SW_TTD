...
# Tạo hàm cho việc phân bổ máy trạm dựa trên thuật toán lớn nhất sau đây 
...
def generate_assembly_line(file_path, env, feasable_solution, Workstation, que, switch):
    ...
    base = pd.read_excel(file_path,sheet_name='cap_ultra_test',skiprows=3,usecols='B:H')
...
# Taọ môi trường chính cho mô phỏng 
...
class Assembly_Line:
    ...
    def __init__(self, file_path, feasable_solution, Workstation, data):
        ...
        self.data = data
        self.file = pd.read_excel(file_path,sheet_name='cap_ultra_test',skiprows=3,usecols='B:H')
...
# Vẽ dây chuyền lắp ráp với ánh xạ màu theo mỗi máy trạm và lưu kết quả 
